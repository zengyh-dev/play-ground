...
LoadCubemapImages(const string& cubemapDir,
    int& width,
    int& height,
    int& channel)
{
    vector<string> cubemapNames { "negx.jpg", "posx.jpg", "posy.jpg",
...
vector<Eigen::Array3f>
PrecomputeCubemapSH(const vector<unique_ptr<float[]>>& images,
    const int& width,
    const int& height,
    const int& channel)
{
    ...
    // 存储球谐系数的向量，每个球谐系数都是一个Eigen::Array3f类型的数组
            ...
            for (int x = 0; x < width; x++) {
                ...
                for (int l = 0; l <= SHOrder; l++) {
                    for (int m = -l; m < l; m++) {
                        ...
                        SHCoeffiecents[index] += Le * sh * weight;
                    }
                }
            }
        }
...
public:
    ...
    string toString() const { return "PRTIntegrator[]"; }
