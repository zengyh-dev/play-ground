...
float InitRand(vec2 uv) {
    vec3 p3 = fract(vec3(uv.xyx) * .1031f);
...
float GetDepth(vec3 posWorld) {
    float depth = (vWorldToScreen * vec4(posWorld, 1.0f)).w;
    return depth;
}
...
vec2 GetScreenCoordinate(vec3 posWorld) {
    vec2 uv = Project(vWorldToScreen * vec4(posWorld, 1.0f)).xy * 0.5f + 0.5f;
    return uv;
}

vec3 GetScreenCoordinate3(vec3 posWorld) {
    return Project(vWorldToScreen * vec4(posWorld, 1.0f)).xyz * 0.5f + 0.5f;
}
...
vec3 EvalDiffuse(vec3 wi, vec3 wo, vec2 uv) {
    ...
    float cos = max(0.f, dot(normal, wi));
...
bool RayMarch(vec3 ori, vec3 dir, out vec3 hitPos) {
    ...
    for(int curStepTimes = 0; curStepTimes < totalStepTimes; curStepTimes++) {
        ...
        if(rayDepth - gBufferDepth > 0.0001f) {
            hitPos = curPos;
            return true;
        }
...
bool RayMarch_Hiz_In_Texture_Space(vec3 start, vec3 rayDir, float maxTraceDistance, out vec3 hitPos) {
    vec2 crossStep = vec2(rayDir.x >= 0.f ? 1 : -1, rayDir.y >= 0.f ? 1 : -1);
    // vec2 crossOffset = crossStep / vec2(1024.0, 1024.0) / 128.;
    vec2 crossOffset = crossStep / vec2(2560.0f, 1440.0f) / 128.f;
    crossStep = clamp(crossStep, 0.0f, 1.0f);
    ...
    ray = intersectCellBoundary(o, d, rayCell, startCellCount, crossStep, crossOffset * 64.f);

    int level = startLevel;
    int iter = 0;
    bool isBackwardRay = rayDir.z < 0.f;

    float Dir = isBackwardRay ? -1.f : 1.f;
    ...
    while(level >= stopLevel && ray.z * Dir <= maxZ * Dir && iter < 100) {
        ...
        float thickness = level == 0 ? (ray.z - cell_minZ) : 0.f;
    ...
    intersected = true;
    hitPos = intersected ? ray : vec3(0.0f);
    return intersected;
}
...
void main() {
  ...
  // L = (GetGBufferDiffuse(screenUV) + EvalReflect(wi, wo, screenUV))/2.;
    ...
    vec3 color = pow(clamp(L, vec3(0.0f), vec3(1.0f)), vec3(1.0f / 2.2f));
    FragColor = vec4(vec3(color.rgb), 1.0f);
}
