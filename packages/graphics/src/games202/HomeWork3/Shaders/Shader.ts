...
export class Shader {
    public gl: WebGL2RenderingContextExtend;
    public program: WebglProgram | null = null;
    constructor(gl: WebGL2RenderingContextExtend, vsSrc: string, fsSrc: string, shaderLocations: ShaderLocations) {
        this.gl = gl;
        // 创建并编译着色器对象
        ...
        if (!glShaderProgram) {
            console.error("linkShader error!");
            return;
        }
    ...
    linkShader(vs: WebGLShader, fs: WebGLShader) {
        const gl = this.gl;
        // 4. 创建着色器程序对象
        // 由vs和fs两个着色器组成，和普通的js程序不一样，它运行在webgl系统中
