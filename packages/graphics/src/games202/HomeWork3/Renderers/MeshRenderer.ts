...
export class MeshRender {
    ...
    constructor(gl: WebGL2RenderingContextExtend, mesh: Mesh, material: Material) {
        this.gl = gl;
        this.mesh = mesh;
    ...
    bindGeometryInfo() {
        ...
        if (this.mesh.normalsName && this.shader.program.attribs[this.mesh.normalsName] >= 0) {
            const numComponents = 3;
            const type = gl.FLOAT;
            const normalize = false;
            const stride = 0;
        ...
        if (this.mesh.texcoordsName && this.shader.program.attribs[this.mesh.texcoordsName] >= 0) {
    ...
    updateMaterialParameters(parameters: any) {
        if (parameters == null) {
    ...
    draw(camera: THREE.Camera, fbo: ExtendedFramebuffer | null, updatedParamters: any) {
        const gl = this.gl;

        // 如果buffer为null，就会解除帧缓冲区的绑定
