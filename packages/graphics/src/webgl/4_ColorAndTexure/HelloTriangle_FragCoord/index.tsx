...
function HelloTriangle_FragCoord() {
    const initVertexBuffers = (gl: WebGLRenderingContextExtend) => {
        ...
        const u_Width = gl.getUniformLocation(gl.program as WebGLProgram, "u_Width");
        if (!u_Width) {
            console.log("Failed to get the storage location of u_Width");
            return -1;
        }

        const u_Height = gl.getUniformLocation(gl.program as WebGLProgram, "u_Height");
        if (!u_Height) {
...
export default HelloTriangle_FragCoord;
